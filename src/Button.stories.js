...
export const buttonkKnobs = () => {
  const label = 'Colors'
  const options = ['primary', 'secondary', 'tertiary', 'outline', 'primaryOutline', 'secondaryOutline']

  const value = select(label, options, 'tertiary')
  return <Button appearance={value}>{value}</Button>
}
